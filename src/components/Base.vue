<template>
  ...
  <img :class="{ focusImg: focusAn }" id="bgImg" :src="imgUrl" alt="" />
  <div class="titleBar">
    <div class="title">
  ...
  <div class="iconBox">
    <el-badge
      ...
      is-dot
      class="item"
      ><i @click="goDownloading" class="iconfont icon-yunduanxiazai"></i
    ></el-badge>
    <router-link to="/home"
      ><i v-if="$route.path != '/home'" class="iconfont icon-zhuye"></i
    ></router-link>
    <router-link to="/setup"
      ><i v-if="$route.path != '/setup'" class="iconfont icon-shezhi"></i
    ></router-link>
  </div>
</template>
...
const imgUrl =
  new URL(
    path.join(path.resolve(), `resources/src/assets/${imgName}`),
    import.meta.url
  ).href +
  "?" +
  localStorage.getItem("endArg");
